...
// Constructor
Receiver::Receiver(int kq, int listen_fd, HttpRequestParser* hrp) : _kq(kq), _listen_fd(listen_fd), _hrp(hrp) {
...
void Receiver::_readEvent(std::vector<EventInfo *>& event_list, EventInfo& event_info){
    ...
    std::cout << "Server: receive from client: '" << buffer <<"' length: " << strlen(buffer)  << std::endl;
